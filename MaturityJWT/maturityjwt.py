...
class MaturityJWT:
    ...
    def provide_access_token(self,login_details,student=0):
        ...
        if student == 0:
            email_exists = self.Maturitycrud.check_exists(("*"),"users",condition=condition)
        elif student == 1:
            email_exists = self.Maturitycrud.check_exists(("*"),"studentsubscriptions",condition=condition)
        if email_exists:
            encrypted_password =  hashlib.sha256(login_details["password"].encode('utf-8')).hexdigest()
            email_data = self.Maturitycrud.get_data(("email","password"),"users",condition=condition)[0]
